defmodule CatalystWeb.Pages.ComputerPage do
  ...
  The shell owns its global computer-use preference toggle. This page owns its
  refresh, settings, and async backend events through the registered page
  lifecycle.
  ...
  **Backend state is queried on navigation and on explicit Refresh, never
  ...
  @doc false
  def handle_async(:computer_panel, {:ok, snapshot}, socket),
    do: {:noreply, assign(socket, computer_panel: snapshot)}

  def handle_async(:computer_panel, {:exit, reason}, socket),
    do: {:noreply, assign(socket, computer_panel: failed_state(reason))}
  ...
  defp refresh(socket) do
    ...
    case pending.available? and connected?(socket) do
      true -> start_async(socket, :computer_panel, &backend_state/0)
      false -> socket
    end
  end
