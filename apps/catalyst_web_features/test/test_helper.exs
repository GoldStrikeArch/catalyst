await_extension_bootstrap = fn await_extension_bootstrap, attempts ->
  case :sys.get_state(Catalyst.Extensions) do
    ...
    state ->
      raise "extension bootstrap did not complete before web feature tests: #{inspect(state)}"
  end
end
...
:ok = Catalyst.LLM.Registry.register_provider(codex_api, Catalyst.LLM.Demo)
