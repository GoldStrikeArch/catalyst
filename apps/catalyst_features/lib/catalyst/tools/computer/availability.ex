defmodule Catalyst.Tools.Computer.Availability do
  ...
  @doc false
  @spec granted?(map()) :: boolean()
  def granted?(config) when is_map(config) do
    enabled? = option(config, :computer_use, app_default(config))
    enabled? == true and available?()
  end
  ...
  defp app_default(config) do
    case option(config, :agent_depth, 0) do
