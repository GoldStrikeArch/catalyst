defmodule Catalyst.ACP.Workflow do
  ...
  @impl true
  def session_backend(selection, opts) do
    {:external, :acp, acp_agent_identity(selection, opts)}
  end
  ...
  defp acp_agent_identity(selection, opts) when is_list(opts) do
  ...
  defp acp_agent_identity(selection, opts) when is_map(opts) do
    case Map.fetch(opts, :acp_agent) do
      {:ok, %{id: id}} when is_binary(id) -> id
      {:ok, %{"id" => id}} when is_binary(id) -> id
      {:ok, configured} -> {:configured, configured}
      :error -> selected_acp_agent(selection)
