defmodule Catalyst.WorkflowRunTest do
  ...
  test "failed initial supervisor start leaves an explicitly resumable checkpoint" do
    ...
    assert :ok = Supervisor.terminate_child(Catalyst.Features.WorkflowSupervisor, supervisor)

    on_exit(fn ->
      case Process.whereis(supervisor) do
        nil -> Supervisor.restart_child(Catalyst.Features.WorkflowSupervisor, supervisor)
        _pid -> :ok
      end
    end)
