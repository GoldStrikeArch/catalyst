...
- **Sidebar order + compare project chooser — DONE ✅.** (1) Switching threads no longer
  ...
  current shell project preselected) with an "Other folder…" option that reveals the path field;
  if session startup leaves no known projects the plain field remains. Tests: `catalog_test`
  (created_at fixed/legacy), new `shell_live/threads_test.exs` (order invariant under focus),
  `comparison_live_test` (current/known project options, preselection, other-folder path feeds
  creation).
